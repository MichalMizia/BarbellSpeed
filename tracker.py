# ------------------------ Import Necessary LibaVelocityListries -------------------------
...
# weight and boolean to control if weightxreps is shown in recording
weight = 190
show_weight = False
# Initialize Aruco size
global aruco_size
aruco_size = 400  # MM

from typing import Tuple, List, Deque

VelocityListType = List[Tuple[int, int, float]]
CoordDequeType = Deque[Tuple[int, int]]


# --------------------------------- Functions -------------------------------------
def is_rack_derack(history: VelocityListType):
    """
...
def calculate_velocity(
    coord_deque: CoordDequeType,
    mmpp,
    velocity_list: VelocityListType,
    rep_rest_time,
    reps,
    analyzed_rep,
    change_in_phase,
...
def showInMovedWindow(winname, img, x, y, out):
    """
    ...
    # Create dummy window, move it to (x, y) and resize
...
def showStats(data_df):
    ...
    stats = [
        ...
        (
            "Last AVG Con Velocity",
            "{:.2f} m/s".format(float(data_df["Avg Velocity"])),
            (0, 255, 0),
        ),
        (
            "Last PEAK Con Velocity",
            "{:.2f} m/s".format(float(data_df["Peak Velocity"])),
            (0, 255, 0),
        ),
        (
            "Last Displacement",
            "{:.2f} mm".format(float(data_df["Displacement"])),
            (0, 255, 0),
        ),
    ]
    ...
    if data_df.iloc[0]["Rep"] > 1:
        stats.append(
            (
                "AVG Velocity Loss",
                "{:.2f} %".format(float(data_df["Avg Velocity Loss"])),
                (0, 255, 0),
            )
        )
        stats.append(
            (
                "PEAK Velocity Loss",
                "{:.2f} %".format(float(data_df["Peak Velocity Loss"])),
                (0, 255, 0),
            )
        )
    ...
    for i, (k, v, c) in enumerate(stats):
        text = "{}: {}".format(k, v)
        drawing_functions.textBGoutline(
            blank_frame,
            text,
            (10, 300 - ((i * 40) + 20)),
            scaling=0.5,
            text_color=(drawing_functions.BLACK),
        )
...
def show_set_avg(data_df):
    ...
    stats = [
        ...
        (
            "Average Rep Velocity",
            "{:.2f} m/s".format(float(data_df["Avg Velocity"].mean())),
            (0, 255, 0),
        ),
        (
            "Average Peak Velocity",
            "{:.2f} m/s".format(float(data_df["Peak Velocity"].mean())),
            (0, 255, 0),
        ),
        (
            "Max Velocity Loss",
            "{:.2f} %".format(float(data_df["Avg Velocity"].max()) * 100),
            (0, 255, 0),
        ),
    ]
...
def findAruco(img, marker_size=6, total_markers=50):
    ...
    bbox, ids, rejectedImgPoints = aruco.ArucoDetector(
        dictionary=arucoDict, detectorParams=arucoParam
    ).detectMarkers(gray)
...
def calculate_velocity_averages(avg_velocities, peak_velocities, reps):
    ...
    # If rep >= 2, calculate average, peak, and velocity losses.
        ...
        avg_vel_loss = (
            (avg_velocities[0] - avg_velocities[-1]) / avg_velocities[0] * 100
        )
        peak_vel_loss = (
            (peak_velocities[0] - peak_velocities[-1]) / peak_velocities[0] * 100
        )

        return avg_velocity, peak_velocity, avg_vel_loss, peak_vel_loss


def is_inflection(y_velocity: float, is_eccentric_portion: bool):
    """
    Determines if an inflection has occured. Used in the automatic detection of partial and full repetitions.
    """
    # print("Checking for inflection: ", y_velocity, is_eccentric_portion)
...
def analyze_for_rep(velocity_list, reps):
    ...
    while True:
        ...
        if not eccentric_phase:
            ...
            if eccentric_displacement >= 150 and is_inflection(
                velocity_list[-pos][2], is_eccentric_portion
            ):
        ...
        if not concentric_phase:
            ...
            if (
                concentric_displacement >= 150
                and is_inflection(velocity_list[-pos][2], not is_eccentric_portion)
            ) or (pos == len(velocity_list) and concentric_displacement >= 150):
        ...
        if (
            eccentric_phase
            and concentric_phase
            and abs(concentric_displacement - eccentric_displacement) < 200
        ):
            print(
                "Found rep {}! first: {} mm, second: {} mm".format(
                    reps + 1, eccentric_displacement, concentric_displacement
                )
            )
...
def pixel_to_mm(bbox):
    ...
    mmpp = aruco_perimeter / aruco_size
...
# -------------------------------- Driving Function ----------------------------------------------
def main(
    video_path=None, output="output", set_weight=0, save_data=False, save_folder=""
):
    """
    ...
    if video_path == None:
        ...
        camera_source = f"./video/{video_path}"
    ...
    drawer = Drawer(weight=weight, show_weight=show_weight)
    ...
    out = cv.VideoWriter(
        f"./video/{output}.mp4", cv.VideoWriter.fourcc(*"mpv4"), vid_fps, (480, 854)
    )
    ...
    coords: deque[tuple[int, int]] = deque(
        maxlen=10000
    )  # each coord is (coord_x, coord_y)
    # Each velocity list elem. is x_distance, y_distance, y_velocity
    velocity_list: VelocityListType = []
    avg_velocities = []
    peak_velocities = []
    ...
    while True:
        ...
        if len(bbox) > 0:
            ...
            for markerCorner, markerID in zip(bbox, ids):
                ...
                cv.circle(frame, (cX, cY), 4, drawing_functions.RED, 1)

        # -------------------- Optical Flow --------------------
        if marker_detected and stop_code == False:
            old_corners = np.array(corners, dtype=np.float32)  # type: ignore
            new_corners, status, error = cv.calcOpticalFlowPyrLK(  # type: ignore
                old_gray, gray_frame, old_corners, None, **lk_params  # type: ignore
            )
        ...
        if marker_detected:
            ...
            coords.appendleft((cX, cY))  # type: ignore

            # Find MMPP conversion every three frames
            if update_mmpp == 0:
                mmpp = pixel_to_mm(corners)  # type: ignore
            ...
            if rep:
                ...
                avg_vel, peak_vel, avg_vel_loss, peak_vel_loss = (
                    calculate_velocity_averages(avg_velocities, peak_velocities, reps)
                )
            ...
            if reps > 0:
                # draw velocity
                drawer.draw_velocity(frame, velocity=avg_velocities[-1], corner=3)

            if start_tracking:
                coord_df.loc[len(coord_df.index)] = [cX, cY, reps]  # type: ignore
        ...
        for i in range(1, len(coords)):
            ...
            cv.line(overlay, coords[i - 1], coords[i], (0, 0, 255), 4)
        alpha = 0.4
    ...
    if save_data:
        ...
        filepath = save_folder + f"/velocity_data_{set_weight}kg_{date_format}.csv"
        filepath2 = save_folder + f"/coord_data_{set_weight}kg_{date_format}.csv"
